...
class EntityViewsData extends CoreEntityViewsData {
  ...
  public function getViewsData() {
    ...
    // Add missing reverse relationships for base fields. Workaround for core issue #2706431.
    // Only add reverse relationships if the field has been created in the DB.
    ...
    $entity_reference_fields = array_filter($base_fields, function (BaseFieldDefinition $field) {
      return !$field->hasCustomStorage() && $field->getType() == 'entity_reference'  && !empty($this->tableMapping->getAllFieldTableNames($field->getName()));
    });
      ...
      foreach (array_keys($bundles) as $bundle) {
        ...
        foreach ($field_definitions as $field_name => $field_definition) {
          if ($field_definition instanceof BundleFieldDefinition) {
            ...
            // Add reverse relationships for bundle plugin entity_reference fields.
            // Only add reverse relationships if the field has been created in the DB.
            if (!$field_definition->hasCustomStorage() && $field_definition->getType() == 'entity_reference' && !empty($this->tableMapping->getAllFieldTableNames($field_name))) {
              $entity_reference_fields[$field_name] = $field_definition;
            }
          }
        }
      }
    }
    // Add reverse relationships for both base and bundle plugin entity_reference fields.
